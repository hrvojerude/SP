...
void pacijenti() {
    ...
    pet=new pac[N];
    for(int i=0; i<N; i++) {
        pac*pacijent=new pac;
                generiraj_brojeve(i);
        if(!provjeri_nemoguce_slucajeve)i--;
    }
}

void ispis(pac*pacijent){
...
void unesi_podatke_o_pacijentu(pac*pacijent){
                cout<<"Ime: ";
                cin>>pacijent->ime;
                cout<<"Prezime: ";
...
void dodaj(){
        ...
        for(int i=0;i<N;i++){
                ...
                for(int j=0;j<3;j++){
                        ...
                        cout<<"--------------------------------------------------------\n";
                        pac*pacijent=new pac;
                        memcpy(pacijent,&pet[i],sizeof(pac));
                        
                        unesi_podatke_o_pacijentu(pacijent);
                        
                    enQueue(red,pacijent);
                    
                    memcpy(&pacijenti[br],pacijent,sizeof(pac));
                    br++;
                }
        }
...
void pomocni(){
        ...
        cout<<"=========================\n";
        pom2=new pac[ukupno_pacijenata];      
        glavni_red_brojac=0;
        while(!empty(red)){
                pac*pacijent=new pac;
                ...
                if(pacijent->datum<19631201&&pacijent->ci==2&&pacijent->ei==1&&pacijent->di==2){//3. korak
                        ...
                        memcpy(&pom2[glavni_red_brojac],pacijent,sizeof(pac));
                        glavni_red_brojac++;
                        }
        }
        cout<<"\nISPIS GLAVNOG REDA\n";
        ...
        for(int i=0;i<glavni_red_brojac;i++){
                pac*pacijent=new pac;
                memcpy(pacijent,&pom2[i],sizeof(pac));
                ispis(&pom2[i]);
                enQueue(red,pacijent);
        }
        system("pause");
